...
exports.enhancedNotificationService = exports.EnhancedNotificationService = void 0;
...
class EnhancedNotificationService extends notificationService_1.NotificationService {
    constructor() {
        super();
    ...
    async sendWhatsAppNotification(user, notification) {
        ...
        await axios_1.default.post(process.env.WHATSAPP_API_URL, {
...
Manage notification preferences: ${baseUrl}/settings/notifications
    ...
    generateWhatsAppTemplate(notification) {
        const baseUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
        const typeEmojis = {
            success: '✅',
            warning: '⚠️',
    ...
    // Helper methods
    async getEnhancedUserPreferences(userId) {
        try {
            const preferences = await prisma_1.prisma.userNotificationPreference.findUnique({
                where: { userId }
            });
            if (preferences) {
                return {
        ...
        return {
            ...
            categories: {
                orders: basicPrefs.categories.orders || true,
    ...
    async scheduleNotification(notification, quietHours) {
        try {
            const scheduledFor = this.calculateNextSendTime(quietHours);
            await prisma_1.prisma.scheduledNotification.create({
                data: {
                    userId: notification.userId,
                    title: notification.title,
                    message: notification.message,
    ...
    calculateNextSendTime(quietHours) {
        const now = new Date();
    ...
    async sendOrderStatusUpdate(userId, orderId, status, details) {
    ...
    async sendDeliveryUpdate(userId, trackingNumber, status, location, eta) {
        return this.sendNotification({
            userId,
    ...
    async sendPaymentAlert(userId, amount, status, orderId) {
        return this.sendNotification({
            userId,
    ...
    async sendCriticalAlert(userId, title, message, actionUrl) {
        return this.sendNotification({
            userId,
            title,
            message,
    ...
    // Bulk operations
    async sendBulkMultiChannelNotification(userIds, notification) {
    ...
    async registerPushSubscription(userId, subscription) {
        try {
            ...
            const existing = await prisma_1.prisma.pushSubscription.findFirst({
                where: {
                    userId,
            ...
            if (existing) {
                ...
                return await prisma_1.prisma.pushSubscription.update({
                ...
                return await prisma_1.prisma.pushSubscription.create({
                    data: {
                        userId,
                        endpoint: subscription.endpoint,
                        p256dh: subscription.p256dh,
    ...
    async unregisterPushSubscription(userId, endpoint) {
        try {
            return await prisma_1.prisma.pushSubscription.updateMany({
                where: {
                    userId,
                    endpoint
                },
    ...
    async updateEnhancedPreferences(userId, preferences) {
        try {
            return await prisma_1.prisma.userNotificationPreference.upsert({
                where: { userId },
                update: preferences,
                create: {
                    userId,
...
//# sourceMappingURL=enhancedNotificationService.js.map