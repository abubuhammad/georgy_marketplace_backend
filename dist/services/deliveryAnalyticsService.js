...
exports.DeliveryAnalyticsService = void 0;
...
class DeliveryAnalyticsService {
    ...
    static async getDeliveryAnalytics(timeframe, filters = {}) {
        try {
            const [metrics, agentPerformance, zonePerformance, timeSeriesData, insights] = await Promise.all([
    ...
    static async calculateDeliveryMetrics(timeframe, filters = {}) {
        const whereClause = this.buildWhereClause(timeframe, filters);
        // Get shipment counts by status
        const shipmentCounts = await prisma_1.prisma.shipment.groupBy({
            by: ['status'],
            where: whereClause,
            _count: {
                id: true
        ...
        const inTransitShipments = shipmentCounts.filter(item => [ShipmentStatus.PICKED_UP, ShipmentStatus.IN_TRANSIT].includes(item.status)).reduce((sum, item) => sum + item._count.id, 0);
        // Get delivery times and revenue data
        const deliveryData = await prisma_1.prisma.shipment.findMany({
            where: {
                ...whereClause,
        ...
        const ratingsData = await prisma_1.prisma.shipment.findMany({
            where: {
                ...whereClause,
                status: ShipmentStatus.DELIVERED
            },
        ...
        const returnedShipments = await prisma_1.prisma.shipment.count({
            where: {
                ...whereClause,
    ...
    static async getAgentPerformanceData(timeframe, filters = {}) {
        const whereClause = this.buildWhereClause(timeframe, filters);
        const agentData = await prisma_1.prisma.deliveryAgent.findMany({
            where: {
                status: 'active',
        ...
        return agentData.map(agent => {
            ...
            const averageDeliveryTime = deliveryTimes.length > 0
                ...
                : 0;
            const onTimeDeliveries = shipments.filter(s => s.deliveredAt && s.estimatedDelivery && s.deliveredAt <= s.estimatedDelivery).length;
            ...
            const performanceTrend = recentPerformance > 95 ? 'up' : recentPerformance < 85 ? 'down' : 'stable';
            return {
                agentId: agent.id,
                ...
                lastDelivery: shipments.length > 0 ?
                    shipments.reduce((latest, s) => s.deliveredAt && (!latest || s.deliveredAt > latest) ? s.deliveredAt : latest, null) : null,
                performanceTrend
            };
        });
    }
    // Get zone performance data
    static async getZonePerformanceData(timeframe, filters = {}) {
        const whereClause = this.buildWhereClause(timeframe, filters);
    ...
    static async getTimeSeriesData(timeframe, filters = {}) {
        const whereClause = this.buildWhereClause(timeframe, filters);
        ...
        for (let i = 0; i < daysDiff; i++) {
            ...
            const dayShipments = await prisma_1.prisma.shipment.findMany({
                where: {
                    ...whereClause,
                    createdAt: {
                        gte: currentDate,
            ...
            const onTimeDeliveries = dayShipments.filter(s => s.actualDelivery && s.estimatedDelivery && s.actualDelivery <= s.estimatedDelivery).length;
            timeSeriesData.push({
    ...
    static async generateInsights(timeframe, filters = {}) {
        ...
        if (underperformingZones.length > 0) {
            const worstZone = underperformingZones.reduce((worst, zone) => zone.successRate < worst.successRate ? zone : worst);
            insights.push({
                type: 'warning',
    ...
    static async exportAnalyticsData(timeframe, filters = {}, format = 'csv') {
        try {
    ...
    static async getRealtimeMetrics() {
        ...
        const [activeDeliveries, agentsOnline, todayShipments] = await Promise.all([
            ...
            // Online agents
            prisma_1.prisma.deliveryAgent.count({
                where: {
                    status: 'active',
                    lastActiveAt: {
    ...
    static buildWhereClause(timeframe, filters = {}) {
        const where = {
            createdAt: {
                gte: timeframe.start,
                lte: timeframe.end
...
exports.DeliveryAnalyticsService = DeliveryAnalyticsService;
//# sourceMappingURL=deliveryAnalyticsService.js.map