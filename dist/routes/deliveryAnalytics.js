...
const deliveryAnalyticsService_1 = require("../services/deliveryAnalyticsService");
const auth_1 = require("../middleware/auth");
const roleAuth_1 = require("../middleware/roleAuth");
const router = express_1.default.Router();
// Middleware - require admin or delivery manager roles
router.use(auth_1.authenticateToken);
router.use((0, roleAuth_1.requireRole)(['ADMIN', 'DELIVERY_MANAGER']));
/**
...
router.get('/', async (req, res) => {
    try {
        const { 'timeframe.start': startDate, 'timeframe.end': endDate, partnerId, agentId, zoneId, deliveryType, status } = req.query;
        ...
        const result = await deliveryAnalyticsService_1.DeliveryAnalyticsService.getDeliveryAnalytics(timeframe, filters);
        if (!result.success) {
...
router.get('/metrics', async (req, res) => {
    try {
        ...
        if (status)
            filters.status = status;
        const metrics = await deliveryAnalyticsService_1.DeliveryAnalyticsService.calculateDeliveryMetrics(timeframe, filters);
        res.json({
            success: true,
            data: metrics,
            timeframe,
...
router.get('/agents', async (req, res) => {
    try {
        const { 'timeframe.start': startDate, 'timeframe.end': endDate, agentId, limit = '20', sortBy = 'successRate', sortOrder = 'desc' } = req.query;
        const timeframe = {
            start: startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
            end: endDate ? new Date(endDate) : new Date()
        };
        ...
        if (agentId)
            filters.agentId = agentId;
        const agentPerformance = await deliveryAnalyticsService_1.DeliveryAnalyticsService.getAgentPerformanceData(timeframe, filters);
        // Sort agents
        ...
        // Limit results
        const limitedResults = agentPerformance.slice(0, parseInt(limit));
        res.json({
            success: true,
            data: limitedResults,
...
router.get('/zones', async (req, res) => {
    try {
        const { 'timeframe.start': startDate, 'timeframe.end': endDate, zoneId, limit = '10', sortBy = 'successRate', sortOrder = 'desc' } = req.query;
        const timeframe = {
            start: startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
            end: endDate ? new Date(endDate) : new Date()
        };
        ...
        if (zoneId)
            filters.zoneId = zoneId;
        const zonePerformance = await deliveryAnalyticsService_1.DeliveryAnalyticsService.getZonePerformanceData(timeframe, filters);
        // Sort zones
        const sortField = sortBy;
        ...
        // Limit results
        const limitedResults = zonePerformance.slice(0, parseInt(limit));
        res.json({
            success: true,
            data: limitedResults,
...
router.get('/trends', async (req, res) => {
    try {
        const { 'timeframe.start': startDate, 'timeframe.end': endDate, partnerId, agentId, zoneId, deliveryType, granularity = 'daily' } = req.query;
        const timeframe = {
            start: startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
            end: endDate ? new Date(endDate) : new Date()
        };
        ...
        if (deliveryType)
            filters.deliveryType = deliveryType;
        const timeSeriesData = await deliveryAnalyticsService_1.DeliveryAnalyticsService.getTimeSeriesData(timeframe, filters);
        res.json({
            success: true,
            data: timeSeriesData,
            timeframe,
...
router.get('/insights', async (req, res) => {
    try {
        const { 'timeframe.start': startDate, 'timeframe.end': endDate, partnerId, agentId, zoneId, deliveryType, category, priority } = req.query;
        const timeframe = {
            start: startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
            end: endDate ? new Date(endDate) : new Date()
        };
        ...
        if (deliveryType)
            filters.deliveryType = deliveryType;
        let insights = await deliveryAnalyticsService_1.DeliveryAnalyticsService.generateInsights(timeframe, filters);
...
router.get('/realtime', async (req, res) => {
    try {
        const realtimeMetrics = await deliveryAnalyticsService_1.DeliveryAnalyticsService.getRealtimeMetrics();
        res.json({
            success: true,
...
router.get('/export', async (req, res) => {
    try {
        const { 'timeframe.start': startDate, 'timeframe.end': endDate, partnerId, agentId, zoneId, deliveryType, format = 'csv' } = req.query;
        const timeframe = {
            start: startDate ? new Date(startDate) : new Date(Date.now() - 30 * 24 * 60 * 60 * 1000),
            end: endDate ? new Date(endDate) : new Date()
        };
        ...
        if (deliveryType)
            filters.deliveryType = deliveryType;
        const exportResult = await deliveryAnalyticsService_1.DeliveryAnalyticsService.exportAnalyticsData(timeframe, filters, format);
...
router.get('/agent/:agentId', async (req, res) => {
    try {
        ...
        const { 'timeframe.start': startDate, 'timeframe.end': endDate } = req.query;
        ...
        const [agentPerformance, timeSeriesData] = await Promise.all([
            deliveryAnalyticsService_1.DeliveryAnalyticsService.getAgentPerformanceData(timeframe, filters),
            deliveryAnalyticsService_1.DeliveryAnalyticsService.getTimeSeriesData(timeframe, filters)
        ]);
...
router.get('/zone/:zoneId', async (req, res) => {
    try {
        ...
        const [zonePerformance, timeSeriesData, agentPerformance] = await Promise.all([
            deliveryAnalyticsService_1.DeliveryAnalyticsService.getZonePerformanceData(timeframe, filters),
            deliveryAnalyticsService_1.DeliveryAnalyticsService.getTimeSeriesData(timeframe, filters),
            deliveryAnalyticsService_1.DeliveryAnalyticsService.getAgentPerformanceData(timeframe, filters)
        ]);
...
//# sourceMappingURL=deliveryAnalytics.js.map